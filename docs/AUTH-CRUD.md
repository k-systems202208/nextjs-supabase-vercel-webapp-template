...
このテンプレートには、Supabase Auth のメール/パスワード認証と、ログインユーザー本人だけが操作できる Todo CRUD サンプルを含みます。

- `/auth/login` ログイン
- `/auth/sign-up` サインアップ
- `/auth/confirm` メール確認コールバック
- `/dashboard` 保護ページ + Todo CRUD
- `supabase/schema.sql` todos テーブル + RLS

## Supabase Database

Supabase SQL Editor で `supabase/schema.sql` を実行します。
