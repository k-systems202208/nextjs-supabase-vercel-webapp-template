...
try {
    ...
    $candidateNames = @($RulesetName)
    if ($RulesetName -eq "Protect main") {
        $candidateNames += "main protection"
    }

    $existingRuleset = Find-RulesetByName -Rulesets $rulesets -Names $candidateNames

    if ($null -ne $existingRuleset) {
        $existingRulesetId = [string](Get-ObjectPropertyValue -Object $existingRuleset -Name "id")
        $existingRulesetName = [string](Get-ObjectPropertyValue -Object $existingRuleset -Name "name")
        if ([string]::IsNullOrWhiteSpace($existingRulesetId)) {
            throw "既存RulesetにIDがありません。"
        }

        if ($existingRulesetName -ne $RulesetName) {
            Write-Host "  Legacy Ruleset found: '$existingRulesetName' (ID: $existingRulesetId). Migrating..."
        }
        else {
...
try {
    ...
    $rulesets = @(ConvertFrom-JsonItems -Json $rulesetsJson)
    $verifiedRuleset = Find-RulesetByName -Rulesets $rulesets -Names @($RulesetName)
