...
GitHubへPushした変更を自動でチェックします。このテンプレートでは lint / typecheck / test / build を実行します。

### Vercel

Next.jsアプリを公開する場所です。GitHubとは役割が違います。
...
```text
元のテンプレート
nextjs-supabase-vercel-webapp-template
...
このテンプレートでは最終的に `quality` Jobが成功することを確認します。
...
```text
Issue #12: 在庫検索を追加する
Branch: feature/12-inventory-search
```

このテンプレートでは**mainを直接変更せず、Branch → PR → CI → Squash Merge**を基本にします。
...
```text
このRepositoryは nextjs-supabase-vercel-webapp-template から作成しました。
Issue #12「在庫検索を追加する」を対応してください。

mainへ直接Commitせず、Issue番号入りBranchを作成してください。
実装だけでなく必要なテストとドキュメントも更新してください。
npm run doctor と npm run check を確認し、Pull Request → CI成功まで進めてください。
```
...
- mainへ直接Pushする
- CIが失敗したままMergeする
- `.env.local` の実値をCommitする
- Supabase Secret Key / `service_role` / passwordをCommitする
